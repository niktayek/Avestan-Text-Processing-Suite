...
def match():
    ...
    print(f"Matched OCR: 0 to {end_ocr_index} with manual: 0 to {end_manual_index}")
...
def recursive_match(manual_words, ocr_words, manual_index, ocr_index, error_counter):
    ...
    if single_match(manual_words[manual_index], ocr_words[ocr_index]):
        ...
        best = [memo_val[0], memo_val[1], [(manual_index, ocr_index)] + memo_val[2], memo_val[3] + calculate_error_counter(error_counter)]
